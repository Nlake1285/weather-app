    ...
    <div style={{...styles.card, position: 'relative'}} className="weather-card">
      ...
      <div style={styles.details}>
        ...
        <div style={{display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '10px'}}>
          ...
          <div><strong>UV Index:</strong> {weather.uv_index !== undefined ? weather.uv_index : '-'}</div>
          <div><strong>Visibility:</strong> {isMetric ? visMetric : visImperial}</div>
          <div><strong>Dew Point:</strong> {weather.dewpoint !== undefined ? (isMetric ? `${dewC}°C` : `${dewF}°F`) : '-'}</div>
        </div>
        <div style={{marginTop: 8}}>
          <strong>Sunrise:</strong> {formatTime(weather.sunrise)} &nbsp; <strong>Sunset:</strong> {formatTime(weather.sunset)}
        </div>
        <p style={{marginTop: 8}}>Wind Speed: {isMetric ? windMetric : windImperial}</p>
        <p>
          Wind Direction:
